  ...
  return (
    <section className="py-32 lg:py-40">
      <div className="container mx-auto px-6 lg:px-8">
        ...
        >
          <p className="text-xs uppercase tracking-[0.2em] font-medium mb-6 opacity-60">{t("cta.label")}</p>
          <h2 className="text-display text-4xl md:text-5xl lg:text-6xl tracking-tight leading-[1.1] mb-6 max-w-3xl mx-auto">
            {t("cta.heading")} <em className="text-display italic">{t("cta.headingItalic")}</em>
          </h2>
          <p className="text-lg opacity-70 max-w-xl mx-auto mb-10 leading-relaxed">{t("cta.subtitle")}</p>
          <Button
            size="lg"
            ...
            asChild
          >
            <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer">{t("cta.bookBtn")}</a>
          </Button>
        </motion.div>
      </div>
    </section>
